...
class DiceBase(object):
    def __init__(self, config, send_text=None, send_file=None, send_image=None, get_info_by_wxid=None):
        ...
        self.get_info_by_wxid = get_info_by_wxid
    ...
    def log_get(self, group):
        ...
        if self.send_file:
            self.send_file(os.path.join(dirname, self.logfile, "%s.txt"%group), group)
        else:
            return "无法发送文件。"
    ...
    def mark_get(self, group, key=""):
        dirname, filename = os.path.split(os.path.abspath(sys.argv[0]))
        if self.send_file:
            self.send_file(os.path.join(dirname, self.logfile, "%s_mark_%s.txt" % (group, key)), group)
        else:
            return "无法发送文件。"

    def _get_name(self, wxid):
        if self.get_info_by_wxid:
            name = self.get_info_by_wxid(wxid)
            return name
        else:
            return wxid
    ...
    def _yijing(self, group, wxid):
        ...
        dirname, filename = os.path.split(os.path.abspath(sys.argv[0]))
        if self.send_image:
            self.send_image(os.path.join(dirname, data["url"][0]), group if group else wxid)
            self.send_image(os.path.join(dirname, data["url"][1]), group if group else wxid)
        # 发送结果
    ...
    def _taluo(self, group, wxid):
        ...
        dirname, filename = os.path.split(os.path.abspath(sys.argv[0]))
        if self.send_image:
            self.send_image(os.path.join(dirname, fname), group if group else wxid)
        if self.send_text:
            self.send_text(res, group if group else wxid)
            time.sleep(0.5)
            # 解析
            for i in range(3):
                d = tldata[result[i]]
                r = "【%s%s】\n画面内容：%s\n 释义：%s"%(result[i], "" if zn[i]>0 else "(逆位)", d["画面内容"], d["正位牌释义"] if zn[i]>0 else d["逆位牌释义"])
                self.send_text(r, group if group else wxid)
                time.sleep(0.5)
        else:
            return "无法发送文本。"
