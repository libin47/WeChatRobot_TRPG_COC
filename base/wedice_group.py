...
import requests
import json
from base.wedice_user import WeDiceUser
import re
# from wcferry import Wcf, WxMsg
import time
from bs4 import BeautifulSoup

class WeDiceGroup(WeDiceUser):
    def __init__(self, config, send_text=None, send_file=None, send_image=None, get_info_by_wxid=None):
        super(WeDiceGroup, self).__init__(config, send_text, send_file, send_image, get_info_by_wxid)
        self.cmds_group = [
    ...
    def get_group_answer(self, cmd:str, wxid:str, group:str, sender:str="", refer=""):
        # 处理或记录
        ...
        if cd:
            if not ("super" in cd.keys() and cd["super"]) and (not ("status" in self.data[group].keys() and self.data[group]["status"])):
                return "游戏未开始，请使用.start启动游戏"
            if sender:
                return cd["fun"](group=group, wxid=wxid, cmd=cmd, sender=sender, refer=refer)
            else:
                return cd["fun"](group=group, wxid=wxid, cmd=cmd, sender=wxid, refer=refer)
    ...
    def coc_mark(self, **kwargs):
        ...
        if len(refer)==0:
            ...
            self.mark_input(group, refer, key)
    ...
    def _send_hidden(self, group, wxid, result):
        if self.send_text:
            ...
            self.send_text(f"{selftext}", wxid)
        else:
            name = self._get_name(wxid)
            selftext = "%s暗骰的结果为:%s" % (name, result)
            return selftext
